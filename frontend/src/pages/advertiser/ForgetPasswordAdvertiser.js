...
const ForgetPassword = () => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/advertiser/forgot-password/send-otp', {
        method: 'POST',
  ...
  const handleVerifyOtp = async () => {
    ...
    setStatus({ type: '', message: '' });
    try {
      const response = await fetch('http://localhost:5000/api/advertiser/forgot-password/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleResetPassword = async () => {
    ...
    setStatus({ type: '', message: '' });
    try {
      const response = await fetch('http://localhost:5000/api/advertiser/forgot-password/reset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
