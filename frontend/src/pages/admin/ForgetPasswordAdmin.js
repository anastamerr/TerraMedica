...
const ForgetPassword = () => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/admin/forgot-password/send-otp', {
        method: 'POST',
  ...
  const handleResetPassword = async () => {
    ...
    setStatus({ type: '', message: '' });
    try {
      const response = await fetch('http://localhost:5000/api/admin/forgot-password/reset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
